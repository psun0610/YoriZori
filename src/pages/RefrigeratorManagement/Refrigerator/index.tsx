...
interface IngredientList {
  COLD: IngredientDetailType[];
  FROZEN: IngredientDetailType[];
  OUTSIDE: IngredientDetailType[];
}
...
const Refrigerator = () => {
  ...
  const filteredIngredientList = useMemo(
    () => ({
      ...
      FROZEN: ingredientList.FROZEN.filter(
        ingredient =>
          selectedCategory === 0 || ingredient.categoryId === selectedCategory,
      ),
      OUTSIDE: ingredientList.OUTSIDE.filter(
        ingredient =>
          selectedCategory === 0 || ingredient.categoryId === selectedCategory,
      ),
    }),
   ...
   * 모든 배열을 다 filter 하던 레거시 코드를 사용된 배열만 filter 하도록 수정하여 성능 개선함함
  ...
  const handleDelete = (ingredient: IngredientDetailType) => {
    AxiosAuth.delete(`/fridges/ingredients/${ingredient.id}`).then(() => {
      setIngredientList(prev => {
        const updatedData = {
          ...prev,
          [ingredient.storagePlace]: prev[ingredient.storagePlace].filter(
   ...
   * @param ingredient 클릭한 재료 객체
   */
  const handleSelectIngredient = (ingredient: IngredientDetailType) => {
    // 이미 클릭된 재료가 선택되면 해제
    if (selectedIngredient?.ingredientId === ingredient.ingredientId) {
      setSelectedIngredient(null);
    } else {
  ...
  /**
   * EditDeleteButton이 아닌 곳을 클릭했을 때 동작하는 함수
   * @param event 클릭 이벤트 객체
   */
  const handleClickOutside = (event: MouseEvent) => {
    // 클릭된 곳이 EditDeleteButton이 아니면, 상태를 null로 설정
    if (
      editDeleteBtnRef.current &&
      !editDeleteBtnRef.current.contains(event.target as Node)
    ) {
      setSelectedIngredient(null);
    }
  };
  ...
  }, [selectedIngredient]); // selectedIngredient가 변경될 때마다 이벤트 리스너를 다시 등록

  return (
    <div>
      <S.Container id="wrapper">
        <Category ITEMS={ITEMS} onClick={index => setSelectedCategory(index)} />

        <div style={{ margin: "10px auto 40px" }}>
          <IngredientSection
            title="실온"
            ingredientList={filteredIngredientList.OUTSIDE}
        ...
        <IngredientSection
          title="냉장고"
          ingredientList={[
            ...filteredIngredientList.COLD,
            ...filteredIngredientList.FROZEN,
          ]}
          handleSelectIngredient={handleSelectIngredient}
        />
